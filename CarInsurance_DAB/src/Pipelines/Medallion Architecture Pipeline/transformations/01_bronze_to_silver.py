...
# --- CLEAN TELEMATICS ---
@dp.table(
    name=f"`{catalog}`.{silver_schema}.car_telematics",
    comment="cleaned telematics events",
    table_properties={
        "quality": "silver"
    }
)
@dp.expect("valid_coordinates", "latitude BETWEEN -90 AND 90 AND longitude BETWEEN -180 AND 180")
def telematics():
    return (
        spark.readStream.table(f"`{catalog}`.{bronze_schema}.car_telematics")
...
@dp.expect("valid_policy_no", "policy_no IS NOT NULL")
def policy():
    return (
        spark.readStream.table(f"`{catalog}`.{bronze_schema}.policy")
...
def claim():
    df = spark.readStream.table(f"`{catalog}`.{bronze_schema}.claim")
    return (
...
def customer():
    df = spark.readStream.table(f"`{catalog}`.{bronze_schema}.customer")
...
def training_images():
    df = spark.readStream.table(f"`{catalog}`.{bronze_schema}.training_images")
...
def claim_images():
    df = spark.readStream.table(f"`{catalog}`.{bronze_schema}.claim_images")
